...
void test_PilotInstructions_HandlesMaxDutyCycleInput(void)
{
	...
	PilotInstructions_SetXTimestamp(timestamp0);
	PilotInstructions_SetXTimestamp(timestamp1);
	...
	TEST_ASSERT_FLOAT_WITHIN(ACCEPTABLE_ERROR, 100, ReturnedPilotResult.xPercentage);
}
...
void test_PilotInstructions_HandlesIntermediateDutyCycleInput(void)
{
	...
	TEST_ASSERT_EQUAL(AAQUAD_SUCCEEDED, r);
	TEST_ASSERT_FLOAT_WITHIN(ACCEPTABLE_ERROR, 0, ReturnedPilotResult.xPercentage);
}
