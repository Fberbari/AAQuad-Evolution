...
void setUp(void)
{
	...
	// these calls must exists to solely test the Elevator channel wthout worrying about the others
...
void test_PilotInstructions_HandlesMaxDutyCycleInput(void)
{
	...
	PilotInstructions_SetYTimestamp(timestamp0);
	PilotInstructions_SetYTimestamp(timestamp1);
	...
	TEST_ASSERT_FLOAT_WITHIN(ACCEPTABLE_ERROR, 100, ReturnedPilotResult.yPercentage);
}
...
void test_PilotInstructions_HandlesIntermediateDutyCycleInput(void)
{
	...
	TEST_ASSERT_EQUAL(AAQUAD_SUCCEEDED, r);
	TEST_ASSERT_FLOAT_WITHIN(ACCEPTABLE_ERROR, 0, ReturnedPilotResult.yPercentage);
}
